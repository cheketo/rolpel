	...
	$("#BtnCreate,#BtnCreateNext").on("click",function(e){
		...
		if(validate.validateFields('new_company_form') && validateMaps())
		{
			...
			alertify.confirm(utf8_decode('¿Desea '+confirmText+' ?'), function(e){
				if(e)
				{
					var process		= process_url+'?object=Customer';
					if(BtnID=="BtnCreate")
					{
...
$(function(){
	...
	$("#image").change(function(){
		var process		= process_url+'?action=newimage&object=Customer';
		var haveData	= function(returningData)
		{
...
{
		var process		= process_url+'?object=Customer&action=Getbranchmodal';
		var haveData	= function(returningData)
		{
			//console.log(returningData);
