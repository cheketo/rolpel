...
class Mailer extends PHPMailer
{
    ...
    public function InsertBatchEmail( $AssocID, $Sender, $SenderName, $Receiver, $ReceiverName, $Subject, $HTML, $Files = '', $AltBody = '', $CC = '', $BCC = '' )
    {
				...
				return Core::Insert(
						...
						'sender,sender_name,receiver,receiver_name,subject,message,files,alt_message,cc,bcc,associated_id,creation_date,created_by,organization_id',

						"'" . $Sender . "','" . $SenderName . "','" . $Receiver . "','" . $ReceiverName . "','" . $Subject . "','" . $HTML . "','" . $Attachment . "','" . $AltBody . "','" . $CC . "','" . $BCC . "'," . $AssocID . ",NOW()," . $_SESSION[ CoreUser::TABLE_ID ] . "," . $_SESSION[ CoreOrganization::TABLE_ID ]
		...
		public function SendBatchEmails( $ShowLogs = false )
		{
				...
				if( !$_SESSION[ CoreOrganization::TABLE_ID ] )
				{

						$_SESSION[ CoreOrganization::TABLE_ID ] = '000';

				}

				if( !$_SESSION[ CoreUser::TABLE_ID ] )
				{

						$_SESSION[ CoreUser::TABLE_ID ] = '000';
	    	...
	    	if( $ShowLogs )
				{
						...
						echo 'Excuting emails query.<br><br>';
	    	...
	    	if( $ShowLogs )
				{
						...
						echo 'Emails query executed :<br>' . Core::LastQuery() . '<br><br>';
	    	...
	    	foreach( $Emails as $Email )
	    	{
						...
						if( $ShowLogs )
						{
								...
								echo 'Email N&deg;' . $Email[ 'email_id' ] . ' is being configurated.<br>';
	    			...
	    			$this->isSendmail();
						...
						foreach( $CC as $EmailCC )
						{
								...
								if( $ShowLogs )
								{
										...
										echo 'Adding ' . $EmailCC[ 0 ] . ' as CC.<br>';
						...
						foreach( $BCC as $EmailBCC )
						{
								...
								if( $ShowLogs )
								{
										...
										echo 'Adding ' . $EmailBCC[ 0 ] . ' as BCC.<br>';
						...
						if( $Email[ 'files' ] )
						{
								...
								if( $ShowLogs )
								{
									 ...
									 echo 'Adding files of email N&deg;' . $Email[ 'email_id' ] . '.<br>';
						...
						if( $ShowLogs )
						{
								...
								echo 'Email N&deg;' . $Email[ 'email_id' ] . ' is being sent.<br>';
						...
						if( $Sent )
						{
								...
								if( $ShowLogs )
								{
										...
										echo 'Email N&deg;' . $Email[ 'email_id' ] . ' sent OK.<br>';
								...
								if( $ShowLogs )
								{
										...
										echo 'Email N&deg;' . $Email[ 'email_id' ] . ' has not been sent, there was an ERROR.<br>';
				...
				if( $ShowLogs && count( $Emails ) < 1 )
				{

						echo 'No se encuentran emails pendientes de envio.<br>';
		...
		public function EmailLog( $Sent, $Receiver, $Sender, $Subject, $Message = '', $Files = '', $AssocID = 0 )
    {
	    	...
	    	$LogID = Core::Insert(
						...
						'sender,receiver,subject,message,file,sent,error,associated_id,creation_date,created_by,organization_id',

						"'" . $Sender . "','" . $Receiver . "','" . $Subject . "','" . $Message . "','" . $FileLog . "','" . $Sent . "','" . $Error . "'," . $AssocID . ",NOW()," . $_SESSION[ CoreUser::TABLE_ID ] . "," . $_SESSION[ CoreOrganization::TABLE_ID ]
		...
		function PurchaseEmail( $PurchaseID, $ReceiverAddress, $ReceiverName, $Subject, $Attachments, $Sender = 'ventas@rolpel.com.ar' , $HTML = '<html><body>Orden de Compra RolPel S.R.L.</body></html>', $AltBody = 'Orden de Compra RolPel', $CC = '', $BCC = 'ventas@rolpel.com.ar' )
		{
				...
				if( $this->Batch )
				{
						...
						$Sent = $this->InsertBatchEmail($PurchaseID, $Sender, $SenderName, $ReceiverAddress, $ReceiverName, $Subject, $HTML, $Attachments, $AltBody, $CC, $BCC );
						...
						$this->EmailLog( $Sent, $ReceiverAddress, $Sender, $Subject, $HTML, $Attachments, $PurchaseID );
