			...
			if( $_POST[ 'items' ] > 0 )
			{
					...
					for( $X = 0; $X<$_POST[ 'items' ]; $X++ )
					{
							...
							if( $PurchaseItem[ 'quantity' ] == $PurchaseItem[ 'quantity_delivered' ] )
							{
									...
									// $total = Core::Select( Purchase::TABLE, 'COUNT(*) as total', "status <> 'F' AND purchase_id = " . $PurchaseItem[ 'purchase_id' ] )[ 0 ][ 'total' ];
									//
									// if( $total < 1 )
									if( Core::Select( PurchaseItem::TABLE, 'COUNT(*) as total', 'purchase_id = ' . $PurchaseItem[ 'purchase_id' ] )[ 0 ][ 'total' ] == Core::Select( PurchaseItem::TABLE, 'COUNT(*) as total', "status = 'F' AND purchase_id = " . $PurchaseItem[ 'purchase_id' ] )[ 0 ][ 'total' ] )
									{
